...
public class RemoveRoleCommand extends Command {
    ...
    public RemoveRoleCommand() {
        super("removerole", "remove role to a member", "removerole <member> <role>");
    }
    ...
    public boolean onCommand(GuildMessageReceivedEvent event, String[] args) {
        ...
        embed.infoEmbed("Cargo retirado com sucesso").addOutlineField("Cargo: @" + role.getName(), "adicionado para: " + user.getName()).setAuthor(event.getAuthor()).setFooter(user).timestamp().setThumbnail(user.getAvatarUrl()).send(event.getGuild().getTextChannelById(Settings.LOG_CHANNEL));
        embed.successEmbed("Removido").deleteSend(event.getChannel(), 5);
        return true;
    }
